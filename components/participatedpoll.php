<?php
if(!isset($_SESSION['participated_id'])){
  header('location: mypoll.php');
  exit();
}
$participatedID=mysqli_real_escape_string($conn,$_SESSION['participated_id']) ;
$participatorUsername=mysqli_real_escape_string($conn,$_SESSION['username']) ;

$username=$user['username'];
$userId=$user['id'];
 
$participated="
SELECT *, SUM(v.num_vote) FROM votes v
JOIN polls p ON p.id = v.poll_id
JOIN poll_options po ON po.poll_id =v.poll_id
WHERE user_id='$userId'
AND v.poll_id ='$participatedID' AND v.username='$participatorUsername';
";
$participatedQuery=mysqli_query($conn, $participated);
$participatedArray=mysqli_fetch_assoc($participatedQuery);

$userDetails = [];
$highestVoter = null; 
$maxVotes = 0; 
$allNominees = $participatedArray['option_text'];
...
$userQuery = "SELECT SUM(num_vote) AS total_votes, username
                FROM votes 
                WHERE poll_id='$participatedID' AND username='$participatorUsername'";
...
if ($stmt) {
    $stmt->bind_param("s", $participatedID);
    $stmt->execute();
    $result = $stmt->get_result();
...
<div class=" p-6">
    ...
    <div class="bg-white rounded-lg p-6 mb-8">
        ...
        <div class="mt-4 space-y-2">
            <p class="text-blue-800 font-bold">Poll Name: <span class="font-semibold text-gray-800"><?=$participatedArray['poll_title']?>.</span></p>
            <p class="text-blue-800 font-bold">Poll Description: <span class="italic font-semibold text-gray-800"><?=$participatedArray['poll_description']?>.</span></p>
            <p class="text-blue-800 font-bold">Status: <span class="font-semibold text-gray-800"><?=$participatedArray['poll_status']?>.</span></p>
        </div>
    </div>
    ...
    <div class="bg-blue-50 border border-blue-300 rounded-lg p-6">
        <h3 class="text-xl font-bold text-blue-800">You voted</h3>
        <div class="mt-6 space-y-4">
           
            <div class="flex items-center justify-between">
                <p class="text-lg font-bold text-gray-800"><?=$participatedArray['username']?></p>
    ...
    <div class="bg-yellow-50 border border-yellow-300 rounded-lg p-6">
        ...
        <div class="flex items-center flex-col justify-between">
             <p class="text-lg font-semibold text-blue-600">will be revealed at the end of the poll</p>
            </div>
            <!-- <?php if(isset($draw)):?>
